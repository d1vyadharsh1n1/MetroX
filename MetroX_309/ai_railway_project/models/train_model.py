...
SUPABASE_URL = os.getenv("https://krspitayulylpjcuktex.supabase.co")
SUPABASE_KEY = os.getenv("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imtyc3BpdGF5dWx5bHBqY3VrdGV4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTg2MDE1NDUsImV4cCI6MjA3NDE3NzU0NX0.PoLFkIFrkMSxcqvGl_8wci1GWh8ceqOXRkPFZE9K6Ko")
TABLE_NAME = "history" # UPDATED table name
MODELS_DIR = "models"
...
def preprocess_data(df: pd.DataFrame):
    ...
    # UPDATED: Drop new dayid column along with other identifiers
    df.drop(columns=['id', 'created_at', 'dayid', 'train_id', 'date', 'last_maintenance_date'], inplace=True, errors='ignore')
    
    # Handle categorical variables using one-hot encoding for simplicity
...
def train_status_prediction_model(df: pd.DataFrame):
    ...
    # Ensure all columns exist before trying to access them
    if target not in df.columns:
        logging.error(f"Target column '{target}' not found after preprocessing. Aborting status model training.")
        return
